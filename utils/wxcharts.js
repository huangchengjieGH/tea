...
var config = {
  ...
  colors: ['#ad0606', '#f7a35c', '#434348', '#90ed7d', '#f15c80', '#8085e9'],
...
function drawPointShape(points, color, shape, context) {
  // console.log('shape' + shape);
	...
	if (shape === 'diamond') {
		points.forEach(function (item, index) {
			if (item !== null) {
				context.moveTo(item.x, item.y - 2.5);  //4.5
...
function drawYAxisTitle(title, opts, config, context) {
  ...
  // xAxisHeight: 15,padding: 12,
  // var startX = config.xAxisHeight + (opts.height - config.xAxisHeight - measureText(title));
  var startX = config.xAxisHeight-10;
  // var startY = config.padding + 0.5 * config.fontSize;
  var startY = config.padding;
  // console.log('startX', startX);
  // console.log('y', config.padding + 0.5 * config.fontSize);
  // console.log(opts.height)
	context.save();
	context.beginPath();
	context.setFontSize(config.fontSize);
	context.setFillStyle(opts.yAxis.titleFontColor || '#333333');
  // context.setFillStyle('#ad0606');
...
function drawYAxisTitle_bak(title, opts, config, context) {
  var startX = config.xAxisHeight + (opts.height - config.xAxisHeight - measureText(title)) / 2;
  context.save();
  context.beginPath();
  context.setFontSize(config.fontSize);
...
function drawLineDataPoints(series, opts, config, context) {
	...
	series.forEach(function (eachSeries, seriesIndex) {
		...
		splitPointList.forEach(function (points, index) {
			context.beginPath();
      context.setStrokeStyle(eachSeries.color); //eachSeries.color
...
function drawXAxis(categories, opts, config, context) {
	...
	var maxXAxisListLength = Math.min(categories.length, Math.ceil(validWidth / config.fontSize / 2));  //1.5
...
function drawYAxisGrid(opts, config, context) {
  ...
  var startX = yAxisTotalWidth-10;
	...
	context.beginPath();
	context.setStrokeStyle(opts.yAxis.gridColor || "#000000");
...
function drawYAxis(series, opts, config, context) {
  ...
  // var textX = config.padding + config.yAxisTitleWidth;
  var textX = 0;
	context.stroke();
	context.beginPath();
	context.setFontSize(config.fontSize);
    ...
    context.fillText(item, textX , pos + config.fontSize / 2);
	});
	context.closePath();
	context.stroke();
	...
	legendList.forEach(function (itemList, listIndex) {
		...
		itemList.forEach(function (item) {
          ...
          context.setFillStyle(item.color);
			...
			context.beginPath();
      context.setFillStyle(opts.extra.legendTextColor || item.color);
